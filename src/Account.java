...
public class Account {
    ...
    public void buyStock(Stock stock, int amount) {
        ...
        for (String stockIndiviual : ownList) {
            String[] stockDetails = stockIndiviual.split(";");
            if ((stock.nameStock()).equals(stockDetails[0])) {
                int loc1 = ownedStocks.indexOf(stockDetails[0]);
                ownedStocks = (ownedStocks.substring(0, loc1 + stockDetails[0].length() + 1) + (Integer.parseInt(stockDetails[1]) + amount) + ownedStocks.substring(loc1 + stockIndiviual.length()));
                return;
            }
        }
        ownedStocks += stock.nameStock() + ";" + amount + ",";
    }
    ...
    public void ownedStocks(Stock[] stockMarket) {
        ...
        for (String stock : ownList) {
            ...
            System.out.println("    Individual price: " + format1.format(price) );
    ...
    public double networth(Stock[] stockMarket) {
        ...
        for (String stock : ownList) {
            ...
            networth += Double.parseDouble(String.valueOf(price * Integer.parseInt(stockDetails[1])));
        }
        return networth;
    }
}